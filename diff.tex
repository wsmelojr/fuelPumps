...
%DIF DEL main.old   Wed Sep 11 11:47:45 2019
%DIF ADD main.tex   Wed Sep 11 11:52:44 2019
...
\begin{document}
\title{\DIFdelbegin \DIFdel{Metrological }\DIFdelend Field Surveillance using Blockchains: \\a Study Case regarding Fuel Dispensers in Brazil}
...
\begin{abstract}
Metrological \DIFdelbegin \DIFdel{field surveillance }\DIFdelend \DIFaddbegin \DIFadd{supervision }\DIFaddend is one of the main activities in the context of legal metrology that assures the correct behavior of a measuring instrument.
In all the world, the metrological surveillance of fuel dispensers (or fuel pumps) \DIFaddbegin \DIFadd{placed in the market (i.e., field surveillance) }\DIFaddend constitutes a complex challenge.
Malicious entities can tamper with these measuring instruments to get undue economic advantages.
This practice is particularly common in developing countries where authorities have difficulty in inspecting all the instruments that are \DIFdelbegin \DIFdel{many times }\DIFdelend \DIFaddbegin \DIFadd{typically }\DIFaddend spread in large geographic areas.   
In this paper, we propose a distributed and decentralized framework that uses vehicles as sensors to implement \DIFdelbegin \DIFdel{metrological }\DIFdelend \DIFaddbegin \DIFadd{the field }\DIFaddend surveillance.
...
Furthermore, we develop a case study that uses \DIFaddbegin \DIFadd{the }\DIFaddend Hyperledger Fabric platform to implement and test our idea in a complete blockchain network.
...
Fuel dispensers (or fuel\DIFaddbegin \DIFadd{/petrol }\DIFaddend pumps) are measuring instruments which play a crucial role in the trading of fuel as a consumable good.
In \DIFdelbegin \DIFdel{practically any place }\DIFdelend \DIFaddbegin \DIFadd{most places }\DIFaddend in the world, fuel stations and consumers trade fuel based on measurements given by fuel dispensers.
...
Such practice affects an expressive percent of trade transactions, resulting in \DIFdelbegin \DIFdel{significative }\DIFdelend \DIFaddbegin \DIFadd{significant }\DIFaddend economic losses for all the society \cite{RodriguesFilho2016}.
...
These mechanisms can be activated and deactivated automatically, \DIFdelbegin \DIFdel{becoming }\DIFdelend \DIFaddbegin \DIFadd{making }\DIFaddend fraudulent behavior stealthy and hard \DIFdelbegin \DIFdel{for detecting}\DIFdelend \DIFaddbegin \DIFadd{to detect}\DIFaddend .
...
In this work, we propose a distributed and decentralized surveillance solution to deal with \DIFdelbegin \DIFdel{the described problem}\DIFdelend \DIFaddbegin \DIFadd{fraudulent fuel dispensers}\DIFaddend .
...
Although vehicles embedded meters can be imprecise and inaccurate, the information collected by a large number of vehicles can \DIFdelbegin \DIFdel{be handy to }\DIFdelend indicate fraudulent behavior in fuel dispensers.
...
Blockchains are a truly distributed append-only data structure that assures information integrity while enables automatized workflows by implementing self-executable code (i.e., smart contracts) \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Christidis2016,Vukolic2017a}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Christidis2016}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\begin{itemize}
 \item We present a statistical approach \DIFdelbegin \DIFdel{for performing metrological }\DIFdelend \DIFaddbegin \DIFadd{to perform }\DIFaddend field surveillance using measuring instruments whose uncertainty is unknown. That is an interesting result with several applications in the legal metrology field. We validate our method by simulating refuel events based on real data from metrological supervision activities and inspections.
 \item We propose a \textit{Distributed and Decentralized Surveillance Framework} (DDSF) and instantiate it in two implementation models: the regulated and the free surveillance schemes. We give particular attention to the free model, where vehicles can contribute to \DIFdelbegin \DIFdel{metrological }\DIFdelend \DIFaddbegin \DIFadd{field }\DIFaddend surveillance actively and spontaneously. We also discuss the available technologies for obtaining fuel measurements from a vehicle's tank in refueling events.
 ...
 \item We describe and implement a case study that instantiates our framework to meet the demand from fuel dispensers surveillance in Sao Paulo state, Brazil. We use \DIFaddbegin \DIFadd{the }\DIFaddend HyperLedger Fabric blockchain platform to store measurements from fuel dispensers and vehicles. We also implement a surveillance strategy using smart contracts.
 Finally, we present \DIFdelbegin \DIFdel{experiments result }\DIFdelend \DIFaddbegin \DIFadd{an experiment }\DIFaddend that evaluates both the surveillance strategy efficiency \DIFdelbegin \DIFdel{as so as }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend the blockchain performance.
\end{itemize}
% \subsection{Topics}
% 
% \begin{itemize}
...
In Brazil, information technologies and management systems also have been applied to legal metrology successfully, increasing the efficiency of \DIFdelbegin \DIFdel{the }\DIFdelend processes regarding all \DIFdelbegin \DIFdel{the levels of control \mbox{%DIFAUXCMD
\cite{RodriguesFilho2015a}}\hspace{0pt}%DIFAUXCMD
, consequently optimizing }\DIFdelend \DIFaddbegin \DIFadd{levels of action \mbox{%DIFAUXCMD
\cite{RodriguesFilho2015a}}\hspace{0pt}%DIFAUXCMD
.
These tools optimize }\DIFaddend the allocation of \DIFdelbegin \DIFdel{the resources in the legal control, bringing }\DIFdelend \DIFaddbegin \DIFadd{resources in legal control.
Furthermore, they also bring }\DIFaddend conformity and measurement assurance to the economy\DIFdelbegin \DIFdel{and fair trade to the }\DIFdelend \DIFaddbegin \DIFadd{, promoting fair trading to }\DIFaddend society.

On the other hand, the usage of new technologies also raises still unknown issues, especially in legal metrology\DIFdelbegin \DIFdel{, which demands a certain level of security , preventing }\DIFdelend \DIFaddbegin \DIFadd{.
These issues include concerns about security and privacy.
They demand strategies to prevent }\DIFaddend the fraudulent use of personal information\DIFdelbegin \DIFdel{. 
Traditional security systems have been showing to be a failure when working in connected and interactive networking, as in a smart city context \mbox{%DIFAUXCMD
\cite{Biswas2016}}\hspace{0pt}%DIFAUXCMD
, and the blockchain technology has been applied to surpass the security aspect.
}\DIFdelend \DIFaddbegin \DIFadd{, for instance.
%DIF >  Traditional security systems have been showing to be a failure when working in connected and interactive networking.
%DIF >   (i.e., as in a smart city context \cite{Biswas2016}, and the blockchain technology has been applied to surpass the security aspect.
}\DIFaddend 
...
\caption{Principle of a measurement transducer and a real device \DIFaddbeginFL \DIFaddFL{(adapted from \mbox{%DIFAUXCMD
\cite{Leitao2014a}}\hspace{0pt}%DIFAUXCMD
).}\DIFaddendFL }
\label{f:transducer}
\end{figure}
...
They are also subject of metrological surveillance, where the equipment is \DIFdelbegin \DIFdel{isnpected }\DIFdelend \DIFaddbegin \DIFadd{inspected }\DIFaddend randomly on the field, checking the accordance with the legal requirements.
...
The main reason is for preventing drivers from running out \DIFaddbegin \DIFadd{of }\DIFaddend fuel.
Vehicles do that using different instruments, \DIFdelbegin \DIFdel{since }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend simple analogic fuel gauges \DIFdelbegin \DIFdel{until }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend modern digital displays embedded in the vehicle's panel.
Some vehicle models also use fuel measurements to improve engine performance \cite{Skog2014}.
However, most of the times, such embedded measuring instrument is not designed to be precise \DIFdelbegin \DIFdel{either }\DIFdelend \DIFaddbegin \DIFadd{or }\DIFaddend accurate.
...
Nowadays\DIFaddbegin \DIFadd{, }\DIFaddend one can easily \DIFdelbegin \DIFdel{find }\DIFdelend \DIFaddbegin \DIFadd{obtain }\DIFaddend low-cost OBD monitoring devices on the Internet \DIFaddbegin \DIFadd{by less than U\$ 10}\footnote{\DIFadd{https://www.amazon.com/s?k=obd2+wireless}}\DIFaddend .
...
\DIFdelbegin \DIFdel{The work of }\DIFdelend \DIFaddbegin \DIFadd{Ahmed et al. }\DIFaddend \cite{Ahmed2017} gives an example of this approach.
...
\section{\DIFdelbegin \DIFdel{The Abstract Model}\DIFdelend \DIFaddbegin \DIFadd{Proposing a Supervision Framework}\DIFaddend }
\DIFaddbegin \DIFadd{In this section, we introduce the theoretical basis of our proposal.
...
\DIFaddend \subsection{A basic Metrological Supervision Framework}
...
\caption{\DIFaddFL{A basic metrological supervision framework.}}
\label{f:basicframe1}
\end{figure}

\DIFaddend Any trade transaction of measured goods can involve a conflict of interests.
...
The measuring instrument $\mathcal{M}_k$ belongs to $v_n$ because he is the most interested \DIFaddbegin \DIFadd{party }\DIFaddend in concluding the transaction and make profits from \DIFdelbegin \DIFdel{that}\DIFdelend \DIFaddbegin \DIFadd{it}\DIFaddend .
In practice, $\mathcal{M}_k$ works as a third party, which provides reliability to the transaction $\mathcal{T}$.
However, different factors can compromise $\mathcal{M}_k$'s precision and accuracy (e.g., \DIFdelbegin \DIFdel{unintentional fails}\DIFdelend \DIFaddbegin \DIFadd{defects}\DIFaddend , misbehavior, or even fraud attacks).
...
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=.4\textwidth]{basicframe1}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{A basic metrological supervision framework}}
%DIFAUXCMD
%DIFDELCMD < \label{f:basicframe1}
...
We assume two \DIFdelbegin \DIFdel{conditions as necessary for enhancing this idea}\DIFdelend \DIFaddbegin \DIFadd{necessary conditions for implementing DDSF}\DIFaddend :
\begin{enumerate}
 ...
 \item \textbf{Each consumer has a second measuring instrument $\mathcal{S}_l$ which gives a complementary estimative of measurement related to the traded good.} This requirement can sound a little complex \DIFdelbegin \DIFdel{a the beginning.
  Measuring }\DIFdelend \DIFaddbegin \DIFadd{at first as measuring }\DIFaddend instruments with high precision and accuracy are expensive.
...
Once such conditions are fulfilled, we can \DIFdelbegin \DIFdel{conceive }\DIFdelend \DIFaddbegin \DIFadd{implement }\DIFaddend DDSF as follows.
...
At the same time, the data storage can be read on anytime \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{by }\DIFaddend any stakeholder interested in implementing metrological surveillance using statistical data analyses.
...
\DIFaddbegin \DIFadd{Notice that the described implementation depends on an open data storage service $\mathcal{D}_s$.
...
\caption{The Distributed and Detrological Surveillance Framework (DDSF)\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
\label{f:basicframe2}
\end{figure}
...
Our hypothesis relies on the Law of Large Numbers (LLN) \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{evans2004probability}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Evans2004}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
The LLN \DIFdelbegin \DIFdel{stats }\DIFdelend \DIFaddbegin \DIFadd{states }\DIFaddend that, in an experiment where the number of samples of a random variable (identically distributed) increases, their average converges strongly to their theoretical value.
...
\caption{\DIFaddFL{Comparing a) the regulated model and b) the free model}}\DIFaddFL{.
}\label{f:reg-free}
\end{figure*}
...
\DIFdelbegin %DIFDELCMD < \begin{figure*}[!t]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=.95\textwidth]{reg-free}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Comparing a) the regulated model and b) the free model}}
%DIFAUXCMD
%DIFDELCMD < \label{f:reg-free}
%DIFDELCMD < \end{figure*}
%DIFDELCMD < 
...
That significantly reduces the possibility of a specific entity to take control of the network peers majority. %DIF > evidence database???
...
\hline
\textbf{}                                  & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{l|}{\textbf{fuel dispensers}} %%%
...
We make use of the \DIFdelbegin \DIFdel{implementation }\DIFdelend \DIFaddbegin \DIFadd{BFT orderer }\DIFaddend described in \cite{Sousa2018}.

\begin{figure}[!t]
\centering
\includegraphics[width=.4\textwidth]{fbn}
\caption{\DIFdelbeginFL \DIFdelFL{Performance results in terms of transactions throughput and latency}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The proposed blockchain architecture}\DIFaddendFL .}
\DIFdelbeginFL %DIFDELCMD < \label{f:performance}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{f:fbn}
...
These modes are:
\begin{itemize}
 \item \DIFdelbegin \DIFdel{Informative: }\DIFdelend \DIFaddbegin \textbf{\DIFadd{Informative:}} \DIFaddend the driver types the measurement informed by the fuel dispenser and also the measurement provided by his vehicle fuel gauge/panel. This mode does not imply any cost to the consumer. However, it is very imprecise and subject to measurement and typing errors.
 \item \DIFdelbegin \DIFdel{Measured by the vehicle system: }\DIFdelend \DIFaddbegin \textbf{\DIFadd{Measured by the vehicle system:}} \DIFaddend the app connects to the vehicle (i.e., by using Bluetooth or OBD technologies) and gets the fuel amount estimative provided by the vehicle's embedded computer. This mode can require an OBD adaptor (in case of the vehicle does not offer a Bluetooth interface) and perhaps some specialized training (or service) to configure the app.
 \item \DIFdelbegin \DIFdel{Measured by smart sensors: }\DIFdelend \DIFaddbegin \textbf{\DIFadd{Measured by smart sensors:}} \DIFaddend the app connects to a smart sensor installed into the vehicle's fuel tank. The smart sensor provides highly accurate measurement and can also sign such information, attesting to its authenticity. This mode is the most expensive one, and its deployment usually requires specialized service.
\end{itemize}
...
\subsection{\DIFdelbegin \DIFdel{Experiment results}\DIFdelend \DIFaddbegin \DIFadd{Experimental evaluation}\DIFaddend }
Before presenting our experiment results, we need to discuss some specific parameters which are relevant \DIFdelbegin \DIFdel{in our analysisheuristic}\DIFdelend \DIFaddbegin \DIFadd{to our analysis}\DIFaddend :
\begin{itemize}
    ...
    \item \DIFdelbegin \DIFdel{$-v_{Err}$ and $+v_{Err}$ are the VFMs }\DIFdelend \DIFaddbegin \DIFadd{$v_{Err}$ is the VFM }\DIFaddend measurement error limit \DIFdelbegin \DIFdel{values}\DIFdelend \DIFaddbegin \DIFadd{value}\DIFaddend .
\end{itemize}
...
\subsection{\DIFdelbegin \DIFdel{Complementary discussions}\DIFdelend \DIFaddbegin \DIFadd{Discussions}\DIFaddend }
...
\DIFdelbegin \DIFdel{We consider that our initial results are meaningful.
}\DIFdelend %DIF > We consider that our initial results are meaningful.
We demonstrate the possibility of \DIFdelbegin \DIFdel{proceeding with a metrological }\DIFdelend \DIFaddbegin \DIFadd{a practical }\DIFaddend field surveillance practice by using innovative tools and concepts from information technology.
...
However, we do not see \DIFdelbegin \DIFdel{such an }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend issue as a drawnback in our solution.
...
The case study related to the field surveillance of fuel dispensers is also \DIFdelbegin \DIFdel{symbolic}\DIFdelend \DIFaddbegin \DIFadd{important}\DIFaddend .
...
Drivers want to contribute because \DIFdelbegin \DIFdel{of the fraud impacts }\DIFdelend \DIFaddbegin \DIFadd{frauds impact }\DIFaddend directly on their money.
Vendors want to contribute because \DIFdelbegin \DIFdel{of an }\DIFdelend unfair concurrency is harmful to the business.

We also emphasize that one of the main merits of our idea is its low-cost implementation.
\DIFdelbegin \DIFdel{How }\DIFdelend \DIFaddbegin \DIFadd{As }\DIFaddend we discussed previously, the use of an app by drivers and the acquisition of devices to interface with the vehicle are affordable.
...
Besides, the use of smart contracts to implement surveillance strategies is very \DIFdelbegin \DIFdel{intuitive}\DIFdelend \DIFaddbegin \DIFadd{simple and improves transparency}\DIFaddend .
...
The Metrology Cloud emerges from the idea that information must be available as soon as possible to improve \DIFdelbegin \DIFdel{decisions }\DIFdelend \DIFaddbegin \DIFadd{decision }\DIFaddend making.
